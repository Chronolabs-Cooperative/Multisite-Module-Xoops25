<?php
// $Id: modulesadmin.php 8108 2011-11-06 13:29:13Z beckmi $
//  ------------------------------------------------------------------------ //
...
function xoops_module_list()
{
    ...
    <tr align='center'><th>"._MD_AM_MODULE."</th><th>"._MD_AM_VERSION."</th><th>"._MD_AM_LASTUP."</th><th>"._MD_AM_DOMAINS_IN."</th><th>"._MD_AM_ACTIVE."</th><th>"._MD_AM_ORDER."<br /><small>"._MD_AM_ORDER0."</small></th><th>"._MD_AM_ACTION."</th></tr>
    ";
    ...
    foreach ( $installed_mods as $module ) {
        ...
        foreach ( $domain_list as $key => $mod ) {
            if (  in_array($key, explode('|',$module->getVar('domains'))) ) {
		...
		echo '</td><td><select name="newdomains['.$module->getVar('mid').'][]" value="1" size="4" multiple="1"/>'.$domain_options.'</select>';
		 foreach ( $domain_list as $key => $mod ) 
            if (  in_array($key, explode('|',$module->getVar('domains'))) ) 
				echo "<input type='hidden' name='olddomains[".$module->getVar('mid')."]['$key']' value='$mod'>";
        ...
        if ($module->getVar('dirname') != 'system' && $module->getVar('isactive') == 1) {
            ...
            $extra = '<a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.XOOPS_URL.'/modules/multisite/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>';
            ...
            $extra = '<a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.XOOPS_URL.'/modules/multisite/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>&nbsp;<a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct=modulesadmin&amp;op=uninstall&amp;module='.$module->getVar('dirname').'"><img src="'.XOOPS_URL.'/modules/multisite/images/uninstall.gif" alt="'._MD_AM_UNINSTALL.'" /></a>';
        } else {
            echo '</td><td><input type="checkbox" name="newstatus['.$module->getVar('mid').']" value="1" checked="checked" /><input type="hidden" name="oldstatus['.$module->getVar('mid').']" value="1" />';
            $extra = '<a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.XOOPS_URL.'/modules/multisite/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>';
        }
        ...
        <td>".$extra."&nbsp;<a href='javascript:openWithSelfMain(\"".XOOPS_URL."/modules/multisite/admin.php?fct=version&amp;mid=".$module->getVar('mid')."\",\"Info\",300,230);'>";
        echo '<img src="'.XOOPS_URL.'/modules/multisite/images/info.gif" alt="'._INFO.'" /></a><input type="hidden" name="module[]" value="'.$module->getVar('mid').'" /></td>
        </tr>
        ';
    ...
    foreach($dirlist as $file) {
        ...
        if ( !in_array($file, $listed_mods) ) {
            ...
            <td>
            <a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct=modulesadmin&amp;op=install&amp;module='.$module->getInfo('dirname').'"><img src="'.XOOPS_URL.'/modules/multisite/images/install.gif" alt="'._MD_AM_INSTALL.'" /></a>';
            echo "&nbsp;<a href='javascript:openWithSelfMain(\"".XOOPS_URL."/modules/multisite/admin.php?fct=version&amp;mid=".$module->getInfo('dirname')."\",\"Info\",300,230);'>";
            echo '<img src="'.XOOPS_URL.'/modules/multisite/images/info.gif" alt="'._INFO.'" /></a></td></tr>
            ';
            unset($module);
            $count++;
        }
...
function xoops_module_install($dirname)
{
    ...
    $db =& $GLOBALS["xoopsDB"];
    $reservedTables = array('avatar', 'avatar_users_link', 'block_module_link', 'xoopscomments', 'config', 'configcategory', 'configoption', 'image', 'imagebody', 'imagecategory', 'imgset', 'imgset_tplset_link', 'imgsetimg', 'groups','groups_users_link','group_permission', 'online', 'bannerclient', 'banner', 'bannerfinish', 'priv_msgs', 'ranks', 'session', 'smiles', 'users', 'newblocks', 'modules', 'tplfile', 'tplset', 'tplsource', 'xoopsnotifications', 'banner', 'bannerclient', 'bannerfinish', 'newfeeds', 'policies');
    $module_handler =& xoops_getmodulehandler('module','multisite');
	$feeds_handler =& xoops_getmodulehandler('feeds','multisite');
    if ($module_handler->getCount(new Criteria('dirname', $dirname)) == 0) {
        ...
        if ($error == false) {
            if (!$module_handler->insert($module)) {
				...
				//Feeds Sitemap
				if ($module->getVar('hassitemap') != 0) {
					$sitemap = $module->getInfo('sitemap');
					$msgs[] = sprintf("<strong>%s</strong>",_MD_AM_ADD_FEED_SITEMAP);
			 		foreach($sitemap['sitemap_func'] as $key => $func) {
						$sitemap_obj = $feeds_handler->create();
						$feedname = (isset($sitemap['sitemap_name'][$key]))?$sitemap['sitemap_name'][$key]:$sitemap['sitemap_func'][$key];
						$sitemap_obj->setVar('feed_type', 'sitemap');
						$sitemap_obj->setVar('feed_name', $feedname);
						$sitemap_obj->setVar('mid', $newmid);						
						$sitemap_obj->setVar('func_feed', $sitemap['sitemap_func'][$key]);						
						$sitemap_obj->setVar('func_file', $sitemap['sitemap_file'][$key]);
						$sitemap_obj->setVar('xml_buffer_updated', time());												
						$feeds_handler->insert($sitemap_obj);
						if( !$sitemap_obj->getVar('fid') )
							$msgs[] = "&nbsp;&nbsp;".sprintf(_MD_AM_ADD_FEED_FAILED, "<strong>".$feedname."</strong>");
						else 
							$msgs[] = "&nbsp;&nbsp;".sprintf(_MD_AM_ADD_FEED_SUCCESSFUL, "<strong>".$feedname."</strong>");

					}
				}				
				
				//Feeds RSS
				if ($module->getVar('hasrss') != 0) {
					$rssinfo = $module->getInfo('rss');
					$msgs[] = sprintf("<strong>%s</strong>",_MD_AM_ADD_FEED_RSS);
			 		foreach($rssinfo['rss_func'] as $key => $func) {
						$rssobj = $feeds_handler->create();
						$feedname = (isset($rssinfo['rss_name'][$key]))?$rssinfo['rss_name'][$key]:$rssinfo['rss_func'][$key];
						$rssobj->setVar('feed_type', 'rss');
						$rssobj->setVar('feed_name', $feedname);
						$rssobj->setVar('mid', $newmid);						
						$rssobj->setVar('func_feed', $rssinfo['rss_func'][$key]);						
						$rssobj->setVar('func_file', $rssinfo['rss_file'][$key]);
						$rssobj->setVar('xml_buffer_updated', time());												
						$feeds_handler->insert($rssobj);
						if( !$rssobj->getVar('fid') )
							$msgs[] = "&nbsp;&nbsp;".sprintf(_MD_AM_ADD_FEED_FAILED, "<strong>".$feedname."</strong>");
						else 
							$msgs[] = "&nbsp;&nbsp;".sprintf(_MD_AM_ADD_FEED_SUCCESSFUL, "<strong>".$feedname."</strong>");
					}
...
function xoops_module_change($mid, $weight, $name, $domains)
{
    $module_handler =& xoops_getmodulehandler('module','multisite');
    $module = $module_handler->get($mid);
    $module->setVar('domains', '|'.implode("|",$domains).'|');
