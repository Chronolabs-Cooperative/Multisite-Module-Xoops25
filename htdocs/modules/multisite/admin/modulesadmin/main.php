...
include_once XOOPS_ROOT_PATH."/modules/multisite/admin/modulesadmin/modulesadmin.php";
$op = "list";
if ( isset($_POST) ) {
...
if ( $op == "confirm" ) {
    ...
    <tr align='center'><th>"._MD_AM_MODULE."</th><th>"._MD_AM_ACTION."</th><th>"._MD_AM_DOMAINS_IN."</th><th>"._MD_AM_ORDER."</th></tr>";
    $mcount = 0;
    ...
    foreach ($module as $mid) {
        ...
        } elseif (in_array($newdomains[$mid], $olddomains[$mid])||count($newdomains[$mid]) != count($olddomains[$mid])) {
            echo "<span style='color:#ff0000;font-weight:bold;'>"._MD_AM_NEWDOMAINS."</span>";
		} else {
            $newstatus[$mid] = 0;
            if ($oldstatus[$mid] == 1) {
        ...
        echo "</td><td align='center'>";		
		if (count($olddomains[$mid]) != count($newdomains[$mid])) {
            echo "<span style='color:#ff0000;font-weight:bold;'>".implode(', ',$newdomains[$mid])."</span>";
        } else {
								
			if (in_array($newdomains[$mid],$olddomains[$mid])) 
				echo "<span style='color:#ff0000;font-weight:bold;'>".implode(', ',$newdomains[$mid])."</span>";		
			else
				echo implode(', ',$newdomains[$mid]);
        }
		...
		foreach ($newdomains[$mid] as $key => $value)
		echo "		<input type='hidden' name='newdomains[".$mid."][".$key."]' value='".$value."' />";
		echo "      <input type='hidden' name='oldweight[".$mid."]' value='".intval($oldweight[$mid])."' />
...
if ( $op == "submit" ) {
    ...
    foreach ($module as $mid) {
        ...
        if ($oldname[$mid] != $newname[$mid] || $oldweight[$mid] != $weight[$mid] || !in_array($newdomains[$mid],$olddomains[$mid]))  {
            $ret[] = xoops_module_change($mid, $weight[$mid], $newname[$mid], $newdomains[$mid]);
            $write = true;
        }
        //flush();
    }
            ...
            foreach ($blocks as $i => $block) {
                if (isset($block['show_func']) && $block['show_func'] != '' && isset($block['file']) && $block['file'] != '') {
                    ...
                    if ($fcount == 0) {
                        ...
                        $result = $xoopsDB->query($sql);
                        if (!$result) {
                            $msgs[] = '&nbsp;&nbsp;'.sprintf(_MD_AM_SQL_NOT_CREATE, $block['name']);
                        } else {
                            if (empty($newbid)) {
